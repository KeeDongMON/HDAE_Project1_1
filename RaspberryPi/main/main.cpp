...
bool send_to_can(const LKASCommand& cmd, int traffic_signal) {
    ...
    frame.data[8] = static_cast<uint8_t>(traffic_signal); // 0:NONE, 1:RED, 2:YELLOW, 3:GREEN
...
int main() {
    ...
    while (true) {
        ...
        for (const auto& c : detected_circles) {
            ...
            Scalar color = Scalar(200,200,200); // default는 옅은 회색
            int thick = 1;
            if (signal != TrafficSignal::NONE) {
                if (signal == TrafficSignal::RED)    color = Scalar(0,0,255);
                if (signal == TrafficSignal::YELLOW) color = Scalar(0,255,255);
                if (signal == TrafficSignal::GREEN)  color = Scalar(0,255,0);
                thick = 3;
            }
            circle(frame, center, radius, color, thick);
        }
        ...
        if (left_curve.size() > 5 && right_curve.size() > 5)
            fill_lane_area(frame, left_curve, right_curve, Scalar(0,255,0), roi_mask);

        draw_roi(frame, roi_verts, Scalar(0,255,255), 2);

        if (left_info.valid)
            draw_curve_in_roi(frame, left_info.poly, roi_mask, Scalar(255,0,0), 3);
        if (right_info.valid)
            draw_curve_in_roi(frame, right_info.poly, roi_mask, Scalar(0,255,0), 3);
        ...
        // 정보 표기
        putText(frame, "Traffic: " + signal_str, Point(10, 40), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(0,180,255), 2);
        ...
        if (lanes_detected) {
            ...
            if (can_socket >=0) {
                if (!send_to_can(lkas, signal_val)) cerr << "CAN send failed" << endl;
            }
            if (can_socket<0 || frame_count%frame_show_gap==0) {
                printf("FPS: %.2f | LKAS: %d Angle: %.1f L: %.1f R: %.1f | Signal: %s [%d]\n",
                       fps, lkas.intervention?1:0, lkas.steering_angle, lkas.left_speed, lkas.right_speed, signal_str.c_str(), signal_val);
            }
            if (lkas.intervention) {
                putText(frame, "LKAS ACTIVE", Point(x,y0), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0,255,255), 1);
                ...
                putText(frame, angle_buf, Point(x,y0+line_gap), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0,255,0), 1);
                putText(frame, speed_buf, Point(x,y0+2*line_gap), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0,200,255), 1);
            } else {
                putText(frame, "LKAS SLEEP", Point(x,y0), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(100,255,100), 1);
                putText(frame, "(Keep Lane)", Point(x,y0+line_gap), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(100,255,100), 1);
            }
        } else {
            putText(frame, "not detected", Point(20, Height-20), FONT_HERSHEY_SIMPLEX, 0.6, Scalar(0,0,255), 2);
            if (can_socket<0 || frame_count%frame_show_gap == 0) {
        ...
        putText(frame, fps_text, Point(20,20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0,255,0), 2);
