...
// RC카 파라미터
const float L = 20.0;      // mm, 앞↔뒤 바퀴 간 거리 (실제 카에 맞게 수정)
const float T = 12.0;      // mm, 좌↔우 바퀴 간 거리 (실제 카에 맞게 수정)
const int MAX_PWM = 255;   // 최대 속도(PWM Duty)
...
void loop() {
  ...
  if (btSerial.connected()) {
    xVal = analogRead(JOY_X_PIN);
    yVal = analogRead(JOY_Y_PIN);
